...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed(&"ui_scale_up"):
		Parameter.set_global(&"all", &"ui_scale", clamp(Parameter.get_global(&"all", &"ui_scale", 3) + 1, 0, UI_SCALE_VALUES.size() - 1))
		refresh_ui_scale()
...
func refresh_ui_scale() -> void:
	ProjectSettings.set_setting("display/window/stretch/scale", UI_SCALE_VALUES[Parameter.get_global(&"all", &"ui_scale", 3)])
